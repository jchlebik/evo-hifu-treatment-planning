 ...
 * 
 * @brief       The source file for solving real value minimization problems with GAUL library Simulated Annealing.
 * 
 * @version     0.4
 * 
                   ...
                   entity *entity)
{
  ...
  // gaul maximizes so we flip the axis
...
void printUsage()
{
  ...
  printf("./... [numberOfIterations] [dimension] [initTemperature] [temperatureStep] [optimalFitness] [rngSeed] [isVerboseOutput]\n");
  printf("Constraints for optimization set in fitness function implementing source file.\n");
}

/*************************************************************************************
 * @brief the main funcion of GAUL Simulated Annealing optimizer.
 * 
...
int main(int argc, char **argv)
{
  ...
  float initTemp = 0.0, tempStep = 0.0;

  // cmd arguments
  iterations      = strtol(argv[1], nullptr, 10);
  kDimension      = strtol(argv[2], nullptr, 10);
  initTemp        = strtof(argv[3], nullptr);
  tempStep        = strtof(argv[4], nullptr);
  kTopScore       = strtof(argv[5], nullptr);
  seed            = strtol(argv[6], nullptr, 10);
  kVerboseOutput  = (bool)strtol(argv[7], nullptr, 10);

  if (iterations < 1 || kDimension < 1 || initTemp < tempStep)
  {
    printUsage();
    return EXIT_FAILURE;
  }

  printf("$dimensionOfProblem:%d$maximumIterations:%d$initTemperature:%g$temperatureStep:%g", 
        kDimension, iterations, initTemp, tempStep);
  ...
  ga_population_set_sa_parameters(
    ...
    initTemp,                   // double init_temp
    0.0,                        // double final_temp
    tempStep,                   // double temp_step
  ...
  ga_sa(
    pop,       // population*   pop
    best,      // entity*       initial
    iterations // const int     max_generations
  );
  ...
  double *chromo = ((double **)best->chromosome)[0];
  printf("\n@@@\n"); // separator of run and results section
  printf("$fitness:%g$time[uS]:%llu$fitnessEvaluations:%d$iterationsNeede:%d$best:", 

                       ...
                       double* chromosome)
{
    ...
    for (unsigned i = 0; i < n; i++)
    {
        ...
        squaredInputsSum += pow(value, 2.0);
        cosSum += cos(gamma * value);
    }

    double inputAvgCoeff = 1.0/(double)n;
    return (20 - 20 * exp(-0.2 *sqrt(inputAvgCoeff * squaredInputsSum)) + M_E - exp(inputAvgCoeff * cosSum));
}

 ...
 * 
 * @brief       The source file of Bohachevsky function fitness score real value optimization.
 * 
 * @version     0.2
 * 
 ...
 *        optimum is at x_i = 0 forall genes, domain defined for all x_i in [-100, 100]
 * 
                       ...
                       double* chromosome)
{
    ...
    for (int i = 0; i < n-1; i++)
    {
        ...
        double value_next = chromosome[i+1];

        score += (pow(value,2) + 2 * pow(value_next, 2) - 0.3* cos(3* M_PI * value) - 0.4 * cos(4.0* M_PI *value_next) + 0.7);
...
{
  return std::fabs(optimum - bestFitness) <= 0.0001f || fitnessEvaluations > 2000;
}
//END of isConverged

function [amp_out, phase_out, compute_time] = fastBeamPatternC(amp_in, phase_in, dx, f0, c0, varargin)
%FASTBEAMPATTERNC Calculate beam pattern using C++ code.
%
% DESCRIPTION:
%     fastBeamPatternC provides an blind interface to the C++ version of
%     fastBeamPattern (called fastBeamPattern-OMP). The function works by
%     appending the optional input 'SaveToDisk' to the user inputs and then
%     calling fastBeamPattern to save the input files to disk. The C++ code
%     is run using the system command. The output files are then
%     automatically loaded from disk and returned in the same fashion as
%     fastBeamPattern. The input and output files are saved to the
%     temporary directory native to the operating system, and are deleted
%     after the function runs. 
%
%     This function requires the C++ binary/executable of 
%     fastBeamPattern-OMP to be downloaded from 
...
%
% USAGE:
%     see fastBeamPattern. Note, the outputs field_t1 and field_t2 are not
%     supported.
%
% OPTIONAL INPUTS:
...
%                       fastBeamPattern-OMP on linux, 
%                       fastBeamPattern-OMP.exe on windows).
...
%
% OPTIONAL OUTPUTS:
%     compute_time    - structure containing the following fields
%                           .total
%                           .save_input
%                           .simulation
%                           .load_output
...
% check the number of outputs, and don't allow for the wave field snapshots
% to be returned
if nargout > 3
    error('The C++ code can only return amp_out and phase_out, not field_t1 and field_t2.');
end
...
if any(strcmp('BinaryName', input_args))
    ...
    if isunix
        binary_name = 'fastBeamPattern-OMP';
    else
        binary_name = 'fastBeamPattern-OMP.exe';
    end
...
fastBeamPattern(amp_in, phase_in, dx, f0, c0, 'SaveToDisk', input_filename, input_args{:});
...
if isunix
    ...
    run_string = ['system(''export LD_LIBRARY_PATH=; cd ' binary_path '; ./' binary_name ' ' input_filename ' ' output_filename ' ' options_string ''' ,''-echo'');'];
    eval(run_string);
    ...
    run_string = ['system(''cd /d ' binary_path ' & ' binary_name ' "' input_filename '" "' output_filename '" ' options_string ''' ,''-echo'');'];
    eval(run_string);
...
amp_out   = h5read(output_filename, '/amp_out');
phase_out = h5read(output_filename, '/phase_out');

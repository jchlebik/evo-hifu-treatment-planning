function test_pass = kWaveDiffusion_compare_2D_heterog_perfusion_with_exact(plot_comparisons, plot_simulations) 
% DESCRIPTION:
...
medium.perfusion_coeff      = perfusion_coeff_1 .* ones(Nx, Ny);
...
s1 = smooth(kgrid, makeDisc(Nx, Ny, Nx/2, Ny/4, 5));
s2 = smooth(kgrid, makeDisc(Nx, Ny, Nx/2, 3*Ny/4, 5));
...
if plot_comparisons
    ...
    axis image;
    colorbar;
    title('k-Wave with Dref max, Pref min (matches left side)');

    subplot(2, 3, 2);
    imagesc(kdiff_min.T);
    axis image;
    colorbar;
    title('k-Wave with Dref min, Pref max (matches right side)');    

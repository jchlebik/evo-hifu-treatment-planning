...
int main(int, char**) {
  ...
  CMAESpp::Parameters<double> parameters;
  ...
  while(!evo.testForTermination())
  {
    ...
    */
    ...
    for (int i = 0; i < evo.get(CMAESpp::CMAES<double>::Lambda); ++i)
      arFunvals[i] = fitfun(pop[i], (int) evo.get(CMAESpp::CMAES<double>::Dimension));

    // update the search distribution used for sampleDistribution()
    evo.updateDistribution(arFunvals);
  }
  std::cout << "Stop:" << std::endl << evo.getStopMessage();
  evo.writeToFile(CMAESpp::CMAES<double>::WKResume, "resumeevo1.dat"); // write resumable state of CMA-ES

  // get best estimator for the optimum, xmean
  xfinal = evo.getNew(CMAESpp::CMAES<double>::XMean); // "XBestEver" might be used as well

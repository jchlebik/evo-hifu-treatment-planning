...
template<typename T>
std::string CMAES<T>::getTimeStr(void)
{
  time_t tm = time(0);
 ...
 */
template<typename T>
void CMAES<T>::eigen(T *diag, T **Q, T *rgtmp)
{
 ...
 */
template<typename T>
int CMAES<T>::checkEigen(T *diag, T **Q)
{
 ...
 */
template<typename T>
void CMAES<T>::householder(T **V, T *d, T *e)
{
  const int n = params.N;
 ...
 */
template<typename T>
void CMAES<T>::sortIndex(const T *rgFunVal, int *iindex, int n)
{
  int i, j;
...
template<typename T>
void CMAES<T>::adaptC2(const int hsig)
{
  const int N    = params.N;
 ...
 */
template<typename T>
void CMAES<T>::testMinStdDevs(void)
{
 ...
 */
template<typename T>
void CMAES<T>::addMutation(T *x, T eps)
{
  for (int i = 0; i < params.N; ++i)
  {
...
template<typename T>
void CMAES<T>::addMutationToGene(T *x, int geneIndex, T eps)
{
  T        sum = 0.0;
  for (int i   = 0; i < params.N; ++i)
  {
 ...
 */
template<typename T>
void CMAES<T>::writeToStream(int key, std::ostream &file)
{
  if (key & WKResume)
  {
 ...
 */
template<typename T>
T *CMAES<T>::init(const Parameters<T> &parameters, const long unsigned seed)
{
  params      = parameters;
  rand        = Random<T>(seed);
  stopMessage = "";
 ...
 */
template<typename T>
void CMAES<T>::resumeDistribution(const std::string &filename)
{
 ...
 */
template<typename T>
T *const *CMAES<T>::samplePopulation()
{
  bool diag = params.diagonalCov==1 || params.diagonalCov >= gen;
 ...
 */
template<typename T>
T *const *CMAES<T>::reSampleSingle(int i)
{
  T *x;
...
template<typename T>
T *const *CMAES<T>::reSampleGene(int popIndex, int geneIndex)
{
  T *x;
 ...
 */
template<typename T>
T *CMAES<T>::sampleSingleInto(T *x)
{
  if (!x)
  {
    x = new T[params.N];
 ...
 */
template<typename T>
T const *CMAES<T>::reSampleSingleOld(T *x)
{
 ...
 */
template<typename T>
T *CMAES<T>::perturbSolutionInto(T *x, T const *pxmean, T eps)
{
  if (!x)
  {
    x = new T[params.N];
 ...
 */
template<typename T>
T *CMAES<T>::updateDistribution(const T *fitnessValues)
{
  const int N    = params.N;
  bool      diag = params.diagonalCov==1 || params.diagonalCov >= gen;
  ...
  if (state==SAMPLED)
  { // function values are delivered here
 ...
 */
template<typename T>
const T *CMAES<T>::getPtr(GetVector key)
{
  switch (key)
  {
    case DiagC:
    ...
    case XBestEver:return xBestEver;
    case XBest:return population[index[0]];
    case XMean:return xmean;
    case FVals:return functionValues;
    default:throw std::runtime_error("getPtr(): No match found for key");
  }
}
 ...
 */
template<typename T>
T *CMAES<T>::getNew(GetVector key, const size_t len)
{
  return getInto(key, len, 0);
}
 ...
 */
template<typename T>
T *CMAES<T>::getInto(GetVector key, const size_t len, T *res)
{
 ...
 */
template<typename T>
bool CMAES<T>::testForTermination()
{
  T                 range, fac;
  int               iAchse, iKoo;
 ...
 */
template<typename T>
std::string CMAES<T>::getStopMessage()
{
  return stopMessage;
}
 ...
 */
template<typename T>
void CMAES<T>::writeToFile(int key, const std::string &filename)
{
  std::ofstream file;
  ...
  if (file.is_open())
  {
    ...
    {
      writeToStream(key, file);
    } /* do not write fitness for gen==0 */
    file.close();
  }
  else
  {
 ...
 */
template<typename T>
void CMAES<T>::updateEigensystem(bool force)
{
  eigenTimings.update();
 ...
 */
template<typename T>
T const *CMAES<T>::setMean(const T *newxmean)
{
...
template class CMAESpp::CMAES<float>;
template class CMAESpp::CMAES<double>;

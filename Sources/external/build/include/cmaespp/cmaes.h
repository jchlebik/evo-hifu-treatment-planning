...
namespace CMAESpp
{
  ...
  public:
     ...
     */
    T* init(const Parameters<T>& parameters, const long unsigned seed = 0);
     ...
     * Request a vector parameter from CMA-ES.
     * @param key Key of the requested vector.
     * @param len Length of the requested vector.
     ...
     */
    T* getNew(GetVector key, const size_t len);

    /**
     * Request a vector parameter from CMA-ES.
     * @param key Key of the requested vector.
     * @param len Length of the requested vector.
     * @param res Memory of size len, where the desired values are
     *            written into. For mem == NULL new memory is allocated as with
     *            calling getNew() and must be released by the user at some point.
     */
    T* getInto(GetVector key, const size_t len, T* res);

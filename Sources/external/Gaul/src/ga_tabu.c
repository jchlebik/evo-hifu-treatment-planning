...
GAULFUNC int ga_tabu(	population		*pop,
  ...
  int		i, j, k;			/* Index into putative solution array. */
 ...
 */
    k = 0; /* endless loop guard */
    i = 0;
    while (i < pop->tabu_params->search_count)
    {
      pop->mutate(pop, best, putative[i]);
      for (j = 0; j < pop->tabu_params->list_length && 
                      tabu_list[j] != NULL && 
                      is_in_tabu == FALSE
                      && k < pop->tabu_params->list_length; j++)
      {
        is_in_tabu = pop->tabu_params->tabu_accept(pop, putative[i], tabu_list[j]);
      }
